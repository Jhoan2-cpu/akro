...
export default function ShiftsIndex({ currentShift, recentShifts, stats }: Props) {
    const [pendingAction, setPendingAction] = useState<'clock-in' | 'clock-out' | null>(null);
    const { errors } = usePage<{ errors: Record<string, string> }>().props;

    const isOpen = stats.openShift;
    const workedToday = useMemo(() => formatMinutes(stats.todayWorkedMinutes), [stats.todayWorkedMinutes]);
                    ...
                    {[
                        {
                            ...
                            hint: isOpen ? 'Debes registrar la salida al finalizar.' : 'Puedes iniciar un turno ahora.',
                            icon: BadgeCheck,
                            tone: isOpen ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-700',
                        },
                        {
                            label: 'Entradas hoy',
                ...
                <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
                    <Card className="border-sidebar-border/70 bg-background shadow-sm">
                        <CardHeader className="space-y-2">
                            <CardTitle className="text-2xl">Estado del turno</CardTitle>
                            <CardDescription>
                                Controla el registro activo y confirma el momento exacto en que inicia o termina.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="rounded-3xl border border-sidebar-border/70 bg-muted/30 p-5">
                                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                    <div>
                                        ...
                                        <p className="mt-1 text-xl font-semibold text-foreground">
                                            {isOpen ? 'Turno abierto' : 'Sin turno abierto'}
                                        </p>
                                    </div>
                                    <Badge
                                        variant="outline"
                                ...
                                {currentShift ? (
                                    <div className="grid gap-3 sm:grid-cols-2">
                                        <div>
                                            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Entrada</p>
                                            <p className="mt-1 text-sm text-foreground">{formatTime(currentShift.clock_in_at)}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Duración parcial</p>
                                            <p className="mt-1 text-sm text-foreground">{currentShift.duration_label}</p>
                                        </div>
                                    </div>
                                ) : (
                                    <p className="text-sm text-muted-foreground">
                                        Cuando registres una entrada, aquí verás la hora exacta de inicio y el tiempo transcurrido.
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="grid gap-3 sm:grid-cols-2">
                                <div className="rounded-3xl bg-emerald-50 p-4 text-emerald-900">
                                    <p className="text-xs font-semibold uppercase tracking-[0.2em]">CU-12</p>
                                    <p className="mt-2 text-sm leading-6">
                                        El sistema bloquea una nueva entrada si ya existe un turno abierto.
                                    </p>
                                </div>
                                <div className="rounded-3xl bg-amber-50 p-4 text-amber-900">
                                    <p className="text-xs font-semibold uppercase tracking-[0.2em]">CU-13</p>
                                    <p className="mt-2 text-sm leading-6">
                                        La salida completa el registro y deja la duración disponible para consulta.
                                    </p>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <Card className="border-sidebar-border/70 bg-background shadow-sm">
                        <CardHeader className="space-y-2">
                            <CardTitle className="text-2xl">Historial reciente</CardTitle>
                            <CardDescription>
                                Los últimos registros de turnos se muestran aquí con hora de entrada, salida y duración.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {recentShifts.length > 0 ? (
                                recentShifts.map((shift) => (
                                    <div key={shift.id} className="rounded-2xl border border-sidebar-border/70 p-4">
                                        <div className="flex items-start justify-between gap-4">
                                            <div>
                                                ...
                                                <p className="text-sm text-muted-foreground">
                                                    Salida: {shift.clock_out_at ? formatTime(shift.clock_out_at) : 'Pendiente'}
                                                </p>
                                            </div>
                                            <Badge
                                                variant="outline"
                                        ...
                                        <div className="mt-3 text-sm text-muted-foreground">
                                            Duración: <span className="font-medium text-foreground">{shift.duration_label}</span>
                                        </div>
                                    </div>
                                ))
                            ) : (
