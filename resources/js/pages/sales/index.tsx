...
export default function SalesIndex({
    ...
    const [ticketPreview, setTicketPreview] = useState<FlashTicket | null>(
        null,
    );

    const saleForm = useForm<{ items: SaleFormItem[]; branch_id?: number }>({
        items: [],
        branch_id: branch?.id,
    });
    ...
    const summary = useMemo(() => {
        return cart.reduce(
            (accumulator, item) => {
                ...
                accumulator.totalTax +=
                    (grossUnitPrice - baseUnitPrice) * quantity;
    ...
    const searchMedicine = async (query: string): Promise<void> => {
        ...
        try {
            ...
            if (!response.ok) {
                setSearchError(
                    payload.message ?? 'No se pudo localizar el medicamento.',
                );
    ...
    const addSelectedMedicine = (): void => {
        ...
        if (quantity > selectedInventory.current_stock) {
            setSearchError(
                'La cantidad supera el stock disponible en la sucursal actual.',
            );
        ...
        setCart((previous) => {
            const nextCart = previous.map((item) => {
                ...
                return {
                    ...item,
                    unit_price: value,
                    is_price_overridden: forceOverride
                        ? true
                        : item.is_price_overridden,
                };
            });
    ...
    const submitSale = (event: React.FormEvent<HTMLFormElement>): void => {
        ...
        if (cart.length === 0) {
            setSearchError(
                'Agrega al menos un medicamento antes de registrar la venta.',
            );
    ...
    const selectedLineTotal =
        selectedMedicine && selectedInventory
            ? Number(draftQuantity || '0') * Number(draftUnitPrice || '0')
            : 0;

    const isOutOfStock = selectedInventory?.is_out_of_stock ?? false;
    const hasWarning = selectedInventory
        ? selectedInventory.is_low_stock || selectedInventory.is_out_of_stock
        : false;
    ...
    return (
        <>
            ...
            <div className="flex w-full justify-start">
                <div className="page-shell-wide flex w-full max-w-none flex-col gap-4 bg-transparent px-4 pt-0 pb-4 md:px-6 md:pt-0 md:pb-6 xl:px-8 xl:pt-0 xl:pb-6 2xl:px-10">
                    <section className="-mx-4 flex w-[calc(100%+2rem)] max-w-none flex-col gap-4 rounded-none border border-primary/20 bg-primary px-5 py-5 text-primary-foreground shadow-xs md:-mx-6 md:w-[calc(100%+3rem)] md:flex-row md:items-start md:justify-between md:px-6 md:py-6 xl:-mx-8 xl:w-[calc(100%+4rem)] 2xl:-mx-10 2xl:w-[calc(100%+5rem)]">
                        <div>
                            ...
                            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-primary-foreground">
                                Venta rápida
                            </h1>
                            <p className="mt-1 max-w-3xl text-sm text-primary-foreground/85 md:text-base">
                                Escanea o busca medicamentos, define cantidad y
                                precio bruto, y confirma la venta sin salir de
                                la pantalla.
                            </p>
                        </div>
                        ...
                        <div className="flex flex-col items-start gap-2">
                            {is_superuser ? (
                                <div className="space-y-1">
                                    <label className="text-xs font-medium tracking-[0.2em] text-primary-foreground/80 uppercase">
                                        Sucursal
                                    </label>
                                    <Select
                                    ...
                                    >
                                        <SelectTrigger className="h-11 w-48 rounded-full border-primary-foreground/20 bg-primary-foreground/10 text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {branches.map((br) => (
                                    ...
                                    {cart.length > 0 && (
                                        <p className="text-xs text-primary-foreground/70">
                                            Vacía el carrito para cambiar de
                                            sucursal
                                        </p>
                                    )}
                                </div>
                                ...
                                <Badge
                                    variant="outline"
                                    className="rounded-full border-primary-foreground/20 bg-primary-foreground/10 text-primary-foreground backdrop-blur-sm"
                                >
                                    {branch
                                        ? branch.name
                    ...
                    {!canSell && (
                        <Alert variant="destructive">
                            ...
                            <AlertDescription>
                                ...
                                Asigna branch_id al empleado antes de usar el
                                módulo.
                            </AlertDescription>
                        </Alert>
                    )}
                            ...
                            <div className="overflow-hidden rounded-3xl border border-sidebar-border/70 bg-background shadow-sm">
                                <div className="bg-primary px-5 py-4 text-primary-foreground md:px-6">
                                    ...
                                    <p className="mt-1 text-sm text-primary-foreground/85">
                                        ...
                                        También puedes escanear con cámara.
                                    </p>
                                </div>
                            ...
                            {selectedMedicine && (
                                <section className="overflow-hidden rounded-3xl border border-sidebar-border/70 bg-background shadow-sm">
                                    <div className="bg-primary px-5 py-4 text-primary-foreground md:px-6">
                                        ...
                                        <p className="mt-1 text-sm text-primary-foreground/85">
                                            Revisa el producto, ajusta la
                                            cantidad y confirma el precio bruto
                                            antes de agregarlo.
                                        </p>
                                    </div>
                                            ...
                                            {selectedInventory && (
                                                <div className="flex flex-wrap gap-2">
                                                    ...
                                                    >
                                                        Mínimo:{' '}
                                                        {
                                                            selectedInventory.minimum_stock
                                                        }
                                                    </Badge>
                                        ...
                                        <div className="mt-6 grid gap-4 md:grid-cols-3">
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    disabled={
                                                        !canSell || isOutOfStock
                                                    }
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-sm font-medium">
                                                    Precio bruto (IVA incl.)
                                                </label>
                                                <Button
                                                    type="button"
                                                    variant="outline"
                        ...
                        <section className="w-full shrink-0 overflow-hidden rounded-3xl border border-sidebar-border/70 bg-background shadow-sm 2xl:w-120">
                            <div className="bg-primary px-5 py-4 text-primary-foreground md:px-6">
                                <div className="flex items-center justify-between gap-3">
                                    ...
                                    <Badge
                                        variant="outline"
                                        className="rounded-full border-primary-foreground/20 bg-primary-foreground/10 text-primary-foreground"
                                    >
                            ...
                            >
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="mt-4 grid gap-3 sm:grid-cols-3">
                                                        <div className="space-y-2">
                                                            <label className="text-xs font-medium tracking-[0.16em] text-muted-foreground uppercase">
                                                                Cantidad
                                                            </label>
                                                            <Input
                                                                type="number"
                                ...
                                <div className="rounded-2xl border border-sidebar-border/70 bg-muted/30 p-4">
                                    ...
                                    <p className="mt-2 text-xs text-muted-foreground">
                                        La venta se registrará con el usuario
                                        autenticado y la sucursal asignada.
                                    </p>
            ...
            >
                <DialogContent className="max-h-[92vh] w-[calc(100vw-1.5rem)] max-w-4xl overflow-hidden p-0">
                    <DialogHeader className="border-b border-sidebar-border/70 px-5 py-4">
                        <DialogTitle>
                            Ticket de venta #{ticketPreview?.sale_id}
                        </DialogTitle>
                        <DialogDescription>
