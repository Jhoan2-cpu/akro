...
export default function Dashboard({
    ...
    const hourlyPoints = useMemo(() => {
        ...
        const selectedBranch = analytics.sales_by_hour_by_branch.find(
            (item) => String(item.branch_id) === hourlyView,
        );
        ...
        analytics.sales_by_hour_by_branch,
        hourlyView,
    ]);
    ...
    const dailyPoints = useMemo(() => {
        ...
        const selectedBranch = analytics.sales_by_day_by_branch.find(
            (item) => String(item.branch_id) === dailyView,
        );
    ...
    return (
        <>
            ...
            <div className="space-y-6 px-4 pt-0 pb-4 md:px-6 md:pb-6 xl:px-8 xl:pb-6 2xl:px-10">
                <section className="-mx-4 w-[calc(100%+2rem)] rounded-none border border-primary/25 bg-primary px-5 py-5 text-primary-foreground shadow-none md:-mx-6 md:w-[calc(100%+3rem)] md:px-6 md:py-6 xl:-mx-8 xl:w-[calc(100%+4rem)] 2xl:-mx-10 2xl:w-[calc(100%+5rem)]">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <Link
                                href="/sales/quick"
                                className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-emerald-900 shadow-md hover:bg-emerald-50"
                            >
                            ...
                            <Link
                                href="/shifts"
                                className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/40 bg-white/10 px-4 py-2 text-sm font-semibold text-white hover:bg-white/20"
                            >
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                        <div className="rounded-2xl border border-emerald-200/70 bg-white p-4">
                            ...
                            <p className="mt-2 text-2xl font-bold text-foreground">
                                {currency.format(kpis.today.total)}
                            </p>
                            <p className="text-xs text-muted-foreground">
                                {change === null
                                    ? 'Sin base comparativa de ayer'
                        ...
                        <div className="rounded-2xl border border-emerald-200/70 bg-white p-4">
                            ...
                            <p className="mt-2 text-2xl font-bold text-foreground">
                                {currency.format(kpis.today.average_ticket)}
                            </p>
                            <p className="text-xs text-muted-foreground">
                                {kpis.today.tickets} ticket(s) emitidos
                            </p>
                        </div>
                        ...
                        <div className="rounded-2xl border border-emerald-200/70 bg-white p-4">
                            ...
                            <p className="mt-2 text-sm font-bold text-foreground">
                                {currency.format(kpis.today.subtotal)}
                            </p>
                            <p className="text-sm text-muted-foreground">
                                IVA: {currency.format(kpis.today.total_tax)}
                            </p>
                        </div>
                        ...
                        <div className="rounded-2xl border border-emerald-200/70 bg-white p-4">
                            ...
                            <p className="mt-2 text-sm font-bold text-foreground">
                                {operations.open_shift
                                    ? 'Abierto'
                                    : 'Sin turno abierto'}
                            </p>
                            <p className="text-xs text-muted-foreground">
                                {operations.open_shift?.clock_in_at ??
                ...
                <section className="grid gap-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)]">
                    <div className="min-w-0 space-y-4">
                        ...
                        <CompactBarChart
                            ...
                            points={dailyPoints}
                            pointLabel={(value, tickets) =>
                                `${currency.format(value)} · ${tickets ?? 0} ticket(s)`
                            }
                        />
                    </div>
                            ...
                            <div className="grid grid-cols-3 gap-2 text-center text-xs">
                                <div className="rounded-lg border border-orange-200 bg-orange-50 p-2">
                                    ...
                                    <p className="text-lg font-bold text-orange-800">
                                        {operations.alerts.low_stock_count}
                                    </p>
                                </div>
                                <div className="rounded-lg border border-rose-200 bg-rose-50 p-2">
                                    ...
                                    <p className="text-lg font-bold text-rose-800">
                                        {operations.alerts.expired_count}
                                    </p>
                                </div>
                                <div className="rounded-lg border border-amber-200 bg-amber-50 p-2">
                                    ...
                                    <p className="text-lg font-bold text-amber-800">
                                        {operations.alerts.near_expiry_count}
                                    </p>
                                </div>
                            </div>

                            <div className="mt-3 space-y-2">
                                {operations.alerts.critical_items
                                    .slice(0, 5)
                                    .map((item) => (
                                        <div
                                            key={item.id}
                                            className="rounded-lg border border-sidebar-border/70 bg-slate-50 px-3 py-2"
                                        >
                                ...
                                {operations.alerts.critical_items.length ===
                                    0 && (
                        ...
                        <section className="rounded-2xl border border-sidebar-border/70 bg-white/95 p-4 shadow-sm">
                            ...
                            <div className="space-y-2">
                                {tasks.map((task, index) => (
                                    <div
                                        key={index}
                                        className="rounded-lg border border-sidebar-border/70 bg-slate-50 px-3 py-2 text-sm text-foreground"
                                    >
                                        {task}
                                    </div>
                                ))}
                ...
                <section className="grid gap-4 lg:grid-cols-2">
                        ...
                        <div className="space-y-2">
                            {analytics.top_medicines.map((item) => (
                                <div
                                    ...
                                    className="flex items-center justify-between rounded-lg border border-sidebar-border/70 px-3 py-2"
                                >
                                    <div>
                                        <p className="text-sm font-semibold text-foreground">
                                            {item.medicine_name}
                    ...
                    <section className="rounded-2xl border border-sidebar-border/70 bg-white/95 p-4 shadow-sm">
                        <div className="mb-3 flex items-center gap-2">
                            ...
                            <h3 className="text-sm font-bold">
                                Riesgo de quiebre (reposiciones)
                            </h3>
                        </div>
                        <div className="space-y-2">
                    ...
                    <section className="rounded-2xl border border-sidebar-border/70 bg-white/95 p-4 shadow-sm">
                        ...
                        <div className="space-y-2 text-sm">
                            <div className="rounded-lg border border-sidebar-border/70 bg-slate-50 px-3 py-2">
                                Ventas con precio manual hoy:{' '}
                                <span className="font-bold text-foreground">
                                    {operations.price_overrides_today}
                                </span>
                            </div>
                            {scope.is_superuser && (
                                <>
                                    {analytics.branch_performance.map(
                                        (item) => (
                                            <div
                                                key={item.branch_id}
                                                className="rounded-lg border border-sidebar-border/70 bg-slate-50 px-3 py-2"
                            ...
                            {!scope.is_superuser && (
                                <div className="rounded-lg border border-sidebar-border/70 bg-slate-50 px-3 py-2 text-xs text-muted-foreground">
                                    El comparativo por sucursal está disponible
